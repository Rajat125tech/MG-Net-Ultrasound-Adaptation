...
def ms_deform_attn_core_pytorch_2d(value, value_spatial_shapes, sampling_locations, attention_weights):
    """
    Core multi-scale deformable attention function in PyTorch for 2D inputs.
    """
    ...
    output = (torch.stack(sampling_value_list, dim=-2).flatten(-2) * attention_weights).sum(-1).view(N_, M_*Dim, Lq_)
    return output.transpose(1, 2).contiguous()


class MSDeformAttn2D(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4):
        super().__init__()
        if d_model % n_heads != 0:
            raise ValueError('d_model must be divisible by n_heads, but got {} and {}'.format(d_model, n_heads))
...
class MSDeformAttnTransformerEncoderLayer2D(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024, dropout=0.1, activation="relu", n_levels=4, n_heads=8, n_points=4):
        super().__init__()
        self.self_attn = MSDeformAttn2D(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
        self.linear1 = nn.Linear(d_model, d_ffn)
        self.activation = _get_activation_fn(activation)
...
class MSDeformAttnTransformerEncoder2D(nn.Module):
    def __init__(self, encoder_layer, num_layers):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers
...
class MSDeformAttnTransformerEncoderOnly2D(nn.Module):
    def __init__(self, d_model=256, nhead=8, num_encoder_layers=6, dim_feedforward=1024, dropout=0.1, activation="relu", num_feature_levels=4, enc_n_points=4):
        ...
        self.d_model = d_model
        self.nhead = nhead
        encoder_layer = MSDeformAttnTransformerEncoderLayer2D(d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, enc_n_points)
        self.encoder = MSDeformAttnTransformerEncoder2D(encoder_layer, num_encoder_layers)
        self.level_embed = nn.Parameter(torch.Tensor(num_feature_levels, d_model))
        self._reset_parameters()
    ...
    def _reset_parameters(self):
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
        for m in self.modules():
            if isinstance(m, MSDeformAttn2D):
                m._reset_parameters()
        nn.init.normal_(self.level_embed)
...
class MSDeformAttnPixelDecoder2D(nn.Module):
    def __init__(
        self,
        input_shape,
        transformer_dropout=0.1,
        ...
        super(MSDeformAttnPixelDecoder2D, self).__init__()
        self.conv_dim = conv_dim
        self.input_shape = input_shape
        self.channel=[input_shape["res3"]['channel'], input_shape["res4"]['channel'], input_shape["res5"]['channel']]
        self.decoder_res3 = self.build_decoder_block(self.channel[0], conv_dim, kernel_size=1)
        ...
        self.transformer = MSDeformAttnTransformerEncoderOnly2D(
            d_model=conv_dim,
            dropout=transformer_dropout,
            nhead=transformer_nheads,
            dim_feedforward=transformer_dim_feedforward,
    ...
    def forward(self, input_tensor):
        res3_features = input_tensor["res3"]
        res4_features = input_tensor["res4"]
        res5_features = input_tensor["res5"]
        srcs_res5 = self.decoder_res5(res5_features)
        srcs_res4 = self.decoder_res4(res4_features)
        srcs_res3 = self.decoder_res3(res3_features)        
        srcs = [srcs_res5, srcs_res4, srcs_res3]

        pos_res5 = self.pe_layer(res5_features)
        pos_res4 = self.pe_layer(res4_features)
        pos_res3 = self.pe_layer(res3_features)
        pos = [pos_res5, pos_res4, pos_res3]
        ...
        out = []
        for i, z in enumerate(y):
            out_i = z.transpose(1, 2).view(bs, -1, spatial_shapes[i][0], spatial_shapes[i][1])
            out.append(out_i)
        return out[::-1]
